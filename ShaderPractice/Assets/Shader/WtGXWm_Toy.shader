Shader"Unlit/WtGXWm_Toy"
{
    ...
    float len)
    {
        Model sr = {d, p, uv, cell, wedges, slice, len};
        return sr;
    }
    ...
    Model leaf(vec3 p, vec3 cellData)
    {
        ...
        if (cellTime > 0.) 
        {
            ...
            len = llen;
            vec2 uv = p.xz / len;
            return NewModel(d, p, uv, cell, wedges, slice, len);
        }

        return NewModel(d, p, vec2(0.,0.), vec2(0.,0.), 0., 0., 0.);
    }
    ...
    vec3 calcAlbedo(Model model) {    
        ...
        col *= mix(vec3(1,1,1), vec3(.5,5.,1.8), smoothstep(.2, 1.8, cell.y) * .75);
    ...
    ) 
    {
        ...
        cell = mul(transformI, cell);
        cell.y = cell.y * (stretch / sz / stretchStart);
    ...
    Model bloom(vec3 p, float t) {
        ...
        mat2 transform = mul(phyllotaxis, mStretch);
        mat2 transformI = inverse(transform);

        Model res = NewModel(1e12, p, vec2(0.,0.), vec2(0.,0.), 0., 0., 0.);
        //res.d = length(p) - 1.; return res;

        // compile speed optim from IQ
        for( int m=min(_iFrame,0); m<3; m++ )
        for( int n=min(_iFrame,0); n<3; n++ )
        {
    ...
    Model map(vec3 p) {
        ...
        if (bound > .01 && 0 == blightingPass) {
            return NewModel(bound, p, vec2(0.,0.), vec2(0.,0.), 0., 0., 0.);
        }
    ...
    float pat(vec2 uv) {
        vec2 p = vec2(atan(uv.x, uv.y), log(length(uv))); //todo
    ...
    void mainImage(out vec4 fragColor, in vec2 fragCoord) {
        ...
        vec2 o = vec2(0, 0);
            ...
            vec3 rayDirection = normalize( mul(camMat, vec3(p.xy,1.8)) );
